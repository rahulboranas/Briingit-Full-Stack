...
const EditCategory = ({close,fetchData,data:categoryData}) => {
    ...
    e.preventDefault()
      ...
      if(responseData.success){
        ...
        close()
        console.log("Calling fetchData after update...") 
        fetchData()
      }
       }catch(error){
    ...
    <section className='fixed top-0 left-0 p-4 right-0 bottom-0 bg-neutral-800/60 flex justify-center items-center'>
        <div className='bg-white max-w-4xl rounded w-full p-4'>
            ...
            <form className='my-3 grid gap-2 ' onSubmit={handleSubmit}>
              ...
              <button className={`${data.name && data.image ? "bg-yellow-400 hover:bg-yellow-300" : "bg-gray-300 "} font-semibold  py-2 `}>Update Category</button>
            </form>
