...
const EditProductAdmin = ({close ,data:propsData , fetchProductData}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await Axios({
                ...SummeryApi.updateProductDetails,
                data: data
            })
    ...
    return (
        <section className='fixed  top-0 right-0 left-0 bottom-0 p-4 z-50 bg-black/70'>
            <div className='bg-white w-full p-4 mx-auto rounded max-w-2xl overflow-y-auto h-full max-h-[95vh] '>
                <section >
                    <div className='gap-3 grid'>
                        <div className='p-2 bg-white shadow-md flex items-center justify-between '>
                            <h2 className='font-semibold'>UploadProduct</h2>
                            <button className='hover:bg-gray-200 p-0.5 rounded-full text-gray-500 hover:text-gray-800' onClick={close}><IoClose size={20}/></button>
                        </div>
